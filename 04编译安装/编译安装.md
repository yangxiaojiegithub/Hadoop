...
## 以安装 tengine-2.1.0.tar.gz 为例
1. 解压 tengine-2.1.0.tar.gz

2. 查看README，找到 Installation
```
Installation
...
安装PCRE库
```
[root@gmall tengine-2.1.0]# yum install pcre-devel
```
...
[root@gmall sbin]# ./nginx
```
在浏览器中访问nginx
![](./doc/nginx访问界面.png)

# rpm 安装: 以安装 jdk-8u65-linux-x64.rpm 为例
** rpm 相关参数 **
```
-a：查询所有套件；
...
```
安装  wget
[root@gmall yum.repos.d]# yum install wget
